...
class Transformer_fusion(nn.Module):
    ...
    def __init__(
        ...
        if self.add_cls:
            self.projection = nn.Sequential(nn.Linear(d_model, 64), nn.ReLU(True), nn.Linear(64, 2))
        else:
            self.projection = nn.Sequential(nn.Linear(d_model * 49, 64), nn.ReLU(True), nn.Linear(64, 2))
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.add_cls:
            combine_emb = torch.cat(
                (self.cls_token.unsqueeze(0).repeat(epitope_emb.size(0), 1, 1), epitope_emb, hla_emb), 1
            )
            combine_padding_mask = torch.cat(
                (self.cls_mask.unsqueeze(0).repeat(epitope_emb.size(0), 1), epitope_padding_mask, hla_padding_mask), 1
            )
        else:
