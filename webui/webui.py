...
with gr.Blocks(title='ImmuBPI') as demo:
    with gr.Row():
        model_manager = gr.Dropdown(choices=list(name_pipe_dict.keys()), value=default_key, label='model', scale=1, min_width=0)
        for i in range(4):
            gr.HTML()
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            from_radio = gr.Radio(choices=['single pair', 'pairs in table'], value='single pair', show_label=False, container=False)
    ...
    with gr.Accordion("Input examples", open=False):
        ...
        example_table = pd.DataFrame.from_dict({
            'peptide': ['VMLQAPLFT', 'VMLQAPLFT', 'NTQTSDTLSK'],
            'hla': ['HLA-A0201', 'HLA-A0101', 'HLA-A0101']
        })
        gr.Examples(
